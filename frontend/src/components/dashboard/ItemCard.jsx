...
export function ItemCard({ item, onEdit, onDelete, onCheck, onZoom }) {
	const getStockStatus = (inStock) => {
		if (inStock === true)
			return {
				label: "In Stock",
				color: "bg-green-500/90 text-white border-green-400/50",
			};
		if (inStock === false)
			return {
				label: "Out of Stock",
		...
		return {
			label: "Unknown",
			color: "bg-zinc-500/90 text-white border-zinc-400/50",
		};
	};
	...
	return (
		<Card className="group overflow-hidden transition-all duration-300 hover:shadow-md border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
			...
			>
				...
				{item.in_stock_confidence !== null &&
					item.in_stock_confidence !== undefined && (
						<div className="absolute bottom-0 left-0 right-0 h-1.5 bg-black/20 backdrop-blur-sm">
							<div
								className={cn(
									"h-full transition-all duration-500",
									item.in_stock_confidence > 0.8
										? "bg-emerald-500"
										: item.in_stock_confidence > 0.5
											? "bg-yellow-500"
											: "bg-red-500",
								)}
				...
				<div
					className={cn(
						"absolute right-3 top-3 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide shadow-sm backdrop-blur-md",
						stockStatus.color,
					)}
				>
					{stockStatus.label}
			...
			<CardContent className="p-5 relative">
				<div className="mb-4 flex items-start justify-between">
					<div className="flex-1 overflow-hidden pr-2">
						...
						<a
							...
							target="_blank"
							rel="noopener noreferrer"
							className="block truncate text-xs text-zinc-400 transition-colors hover:text-primary"
						>
				...
				<div className="space-y-4">
					<div>
						...
						{item.current_price_confidence !== null &&
							item.current_price_confidence !== undefined && (
								<div className="group/confidence relative mt-2">
									<div className="h-1.5 w-full overflow-hidden rounded-full bg-zinc-100 dark:bg-zinc-800">
										<div
											className={cn(
												"h-full rounded-full transition-all duration-500",
												item.current_price_confidence > 0.8
													? "bg-primary"
													: item.current_price_confidence > 0.5
														? "bg-primary/70"
														: "bg-primary/40",
											)}
											style={{
												width: `${item.current_price_confidence * 100}%`,
											}}
										/>
									</div>
									<div className="absolute -bottom-4 left-0 text-[10px] text-muted-foreground opacity-0 transition-opacity group-hover/confidence:opacity-100">
										Price Confidence:{" "}
										{Math.round(item.current_price_confidence * 100)}%
									</div>
								</div>
							)}
					</div>
					...
					{item.tags && (
						<div className="flex flex-wrap gap-1.5">
							{item.tags.split(",").map((tag) => (
								<span
									key={tag}
									className="rounded-md border bg-secondary px-2 py-0.5 text-[10px] font-medium text-secondary-foreground"
								>
									{tag.trim()}
								</span>
							))}
				...
				<div className="flex w-full items-center justify-between">
					...
					<div className="flex gap-1">
						...
						<Button
							...
							disabled={item.is_refreshing}
						>
							<RefreshCw
								className={cn(
									"h-3 w-3",
									item.is_refreshing && "animate-spin text-primary",
								)}
							/>
						</Button>
						<Button
