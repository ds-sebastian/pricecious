...
export function Marquee({ text, className }) {
    ...
    return (
        <div ref={containerRef} className={cn("overflow-hidden w-full group", className)}>
            <div
                ref={textRef}
                className={cn("whitespace-nowrap flex gap-8", isOverflowing && "group-hover:animate-scroll")}
            >
                <span>{text}</span>
