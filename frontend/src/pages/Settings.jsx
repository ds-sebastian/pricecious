	...
	const fetchAll = async () => {
		try {
			...
			setConfig({
				...
				ai_api_base:
					settingsMap["ai_api_base"] !== undefined
						? settingsMap["ai_api_base"]
						: "http://ollama:11434",
				ai_temperature: parseFloat(settingsMap["ai_temperature"] || "0.1"),
				ai_max_tokens: parseInt(settingsMap["ai_max_tokens"] || "300"),
				confidence_threshold_price: parseFloat(
					settingsMap["confidence_threshold_price"] || "0.5",
				),
				confidence_threshold_stock: parseFloat(
					settingsMap["confidence_threshold_stock"] || "0.5",
				),
				enable_json_repair: settingsMap["enable_json_repair"] !== "false",
				smart_scroll_enabled: settingsMap["smart_scroll_enabled"] === "true",
				smart_scroll_pixels: parseInt(
					settingsMap["smart_scroll_pixels"] || "350",
				),
				text_context_enabled: settingsMap["text_context_enabled"] === "true",
				text_context_length: parseInt(
				...
				price_outlier_threshold_percent: parseFloat(
					settingsMap["price_outlier_threshold_percent"] || "500",
				),
			});
		} catch (error) {
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
								<div className="space-y-4">
									...
									<div className="space-y-2">
										...
										<Select
											value={config.ai_reasoning_effort}
											onValueChange={(val) =>
										...
										{config.price_outlier_threshold_enabled && (
											<div className="space-y-2">
												<div className="flex justify-between">
													...
													<span className="text-xs text-muted-foreground">
														{config.price_outlier_threshold_percent}%
													</span>
												</div>
												<Input
													type="number"
								...
								<div className="space-y-4">
									...
									<div className="flex items-center justify-between pt-2">
										...
										<Switch
											id="json_repair"
											checked={config.enable_json_repair}
											onCheckedChange={(checked) =>
					...
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								setJobConfig({
									...jobConfig,
									refresh_interval_minutes: e.target.value,
								})
							}
						/>
					</div>
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										setNewProfile({
											...newProfile,
											price_drop_threshold_percent: parseFloat(e.target.value),
										})
									}
								/>
							</div>
