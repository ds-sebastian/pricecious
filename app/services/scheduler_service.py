...
def _get_thresholds(session: Session):
...
def _update_db_error(item_id: int, error_msg: str):
...
async def process_item_check(item_id: int, semaphore: asyncio.Semaphore | None = None):
    """
    Process a single item check.
    If a semaphore is provided, it will be used to limit concurrency.
    """
    if semaphore:
...
async def _execute_check(item_id: int):
    ...
    try:
        ...
        logger.error(f"Error in process_item_check for item {item_id}: {e}")
...
async def scheduled_refresh():
    ...
    try:
        ...
        # Limit concurrency to avoid overwhelming the system/browser
